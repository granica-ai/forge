...
**Tags** (e.g. `v0.8.3-alpha`) are the supported contract. `main` is updated with each release.
...
```bash
export FORGE_VERSION=v0.8.3-alpha
...
```bash
export FORGE_VERSION=v0.8.3-alpha   # your release tag
...
```bash
export FORGE_VERSION=v0.8.3-alpha   # replace with your tag
kubectl apply -f "https://raw.githubusercontent.com/granica-ai/forge/${FORGE_VERSION}/operator/install.yaml"
```
