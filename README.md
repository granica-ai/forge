...
**Tags** (e.g. `v0.9.4-commit-fix`) are the supported contract. `main` is updated with each release.
...
```bash
export FORGE_VERSION=v0.9.4-commit-fix
...
```bash
export FORGE_VERSION=v0.9.4-commit-fix   # your release tag
...
```bash
export FORGE_VERSION=v0.9.4-commit-fix   # replace with your tag
kubectl apply -f "https://raw.githubusercontent.com/granica-ai/forge/${FORGE_VERSION}/operator/install.yaml"
```
