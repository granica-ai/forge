...
```bash
git clone --depth 1 --branch v0.9.4 https://github.com/granica-ai/forge.git ~/forge
```
...
helm install forge-api ~/forge/helm/forge-api \
  ...
  --set env.FORGE_SPARK_IMAGE=${ACR_NAME}.azurecr.io/crunch:v0.9.4-${ARCH}-spark \
  --set env.FORGE_CRUNCH_IMAGE=${ACR_NAME}.azurecr.io/crunch:v0.9.4-${ARCH} \
